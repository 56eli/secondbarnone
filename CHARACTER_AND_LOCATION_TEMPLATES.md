# Character & Location Asset Templates

**Status:** work in progress
**Last updated:** 2026-07-29

This file tracks the asset pipeline for every character portrait and location
background in the game. Each entry records the current provenance of the
portrait image, the written profile (bio / relationship text), and the
character's events. The long-term goal — stated at the bottom — is to move
every entry to a fully custom state.

---

## Long-term goal

The aspiration for this project is that every character template reaches the
following state:

| Field    | Target        |
|----------|---------------|
| **Sex**     | Known (Male / Female / Other) — no entries remain "unknown" |
| **Image**   | **Custom** — hand-painted or commissioned art, not AI-generated |
| **Profile** | **Custom** — written by a human author, not AI-generated |
| **Events**  | **Custom** — authored dialogue and scenarios, not AI-generated |

The same aspiration applies to locations: every location should reach custom
image, custom description, and a verified location reference.

Progress can be tracked by counting entries that still read "AI-generated" or
"unknown" in each column.

---

## Character templates

78 characters total. 1 protagonist, 1 arch nemesis, 2 rivals, 74 side
characters.

### Protagonist

```
Name: Léon
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

### Arch Nemesis

```
Name: Kaden
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

### Rivals

```
Name: Sato
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Alex
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

### Side Characters

```
Name: Geo
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Lakshay
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Arian
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Simon
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Kaj
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Dorian
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Barret
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Ethan
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Matt
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Artem
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Klaudia
Sex: Female
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Brian
Sex: Male
Image: custom
Profile: AI-generated
Events: AI-generated
```

```
Name: Susan
Sex: Female
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: HawkinsTV
Sex: Other (robot)
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: RicoLewis
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Yun
Sex: unknown
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Marlies
Sex: Female
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Yume
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Hanans
Sex: Female
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Brock Lee
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Tarrasqu
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Kaschem
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Mateo
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Luca
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Vanna
Sex: Female
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Carl-bot
Sex: Other (robot)
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Friend
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: DocBot
Sex: Other (robot)
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Cheezl
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Sir Cruds
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: nestomalt
Sex: unknown
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Qusтoge
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: groovyphoenix
Sex: Other (firebird)
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Cary
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Self
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Daniela
Sex: Female
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Baris
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Aril Stellar☯
Sex: Female
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Alvigunilla
Sex: Female
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Fraghis
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Mrone
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: 𝕽𝖆𝖚𝖑
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Stephen
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Marlène xoxo
Sex: Female
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: diamndsdancin
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Seth
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: SiekamCebulę
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Kopung (고풍)
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Kate
Sex: Female
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Isra
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Kobideh
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: stijn12d
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Andre Watson
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Air-Vaisselle
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Crveni
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: blokely
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Jits
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Gordon
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Ahyeon Oh
Sex: Female
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

> **Note:** The character data file has "Oh" listed as a separate entry from
> "Ahyeon". This is a misparse — the full name is "Ahyeon Oh" and "Oh" is not
> a separate character. The "Oh" entry should be removed from `characters.js`.

```
Name: Oh
Sex: unknown (should be removed — see Ahyeon Oh above)
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Jared
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: emily
Sex: Female
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: RicardoEA
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: SpeedFire
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Orshi
Sex: Female
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Renata 🦥
Sex: Female
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Iulian
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Brendan
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Hazel
Sex: Female
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Scatmandu
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: yungnosaj
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Joar
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Lou
Sex: Female
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

```
Name: Cat
Sex: Male
Image: AI-generated
Profile: AI-generated
Events: AI-generated
```

---

## Location templates

23 locations across 5 districts. Each location has a host from the cast and a
set of tags that drive weather, perk and event interactions.

### Belleville

```
Name: Home Loft
Image: AI-generated
Description: AI-generated
Location reference: Léon's own flat — rest, quiet and recovery; always open from day one
```

```
Name: Rooftop
Image: AI-generated
Description: AI-generated
Location reference: A rooftop in Belleville — outdoor quiet at night, unlocked from day one
```

```
Name: Community Clinic
Image: AI-generated
Description: AI-generated
Location reference: A volunteer clinic in Belleville — community service, unlocked day 3 / 15 reputation
```

```
Name: La Cantine Solidaire
Image: AI-generated
Description: AI-generated
Location reference: A soup kitchen in Belleville — volunteer work and social meals, unlocked day 4 / 20 reputation
```

### Canal Saint-Martin

```
Name: La Maison Calme
Image: AI-generated
Description: AI-generated
Location reference: The spiritual community hall — the founding sanctuary, always open from day one
```

```
Name: Canal Walk
Image: AI-generated
Description: AI-generated
Location reference: A walk along the Canal Saint-Martin — outdoor quiet, unlocked day 2
```

```
Name: Community Garden
Image: AI-generated
Description: AI-generated
Location reference: A shared garden by the canal — outdoor community work, unlocked day 3 / 15 reputation
```

```
Name: Saturday Market
Image: AI-generated
Description: AI-generated
Location reference: Market only open on Saturdays — outdoor market and community work by the canal
```

```
Name: Bathhouse
Image: AI-generated
Description: AI-generated
Location reference: A bathhouse by the canal — rest and quiet, unlocked day 9
```

### Le Marais

```
Name: Le Dernier Verre
Image: AI-generated
Description: AI-generated
Location reference: The bar — the founding money-maker, always open from day one
```

```
Name: Night Market
Image: AI-generated
Description: AI-generated
Location reference: A night market in Le Marais — outdoor market work at night, unlocked day 5 / 25 reputation
```

```
Name: Puces de Saint-Ouen
Image: AI-generated
Description: AI-generated
Location reference: The flea market of Saint-Ouen — outdoor market work, unlocked day 7 / 30 reputation
```

```
Name: Bibliothèque Forney
Image: AI-generated
Description: AI-generated
Location reference: A historic library in Le Marais — study and quiet, unlocked day 4 / 20 reputation
```

```
Name: Verrier, Antiquaire
Image: AI-generated
Description: AI-generated
Location reference: A pawn shop in Le Marais — market and admin, unlocked day 6 / 25 reputation
```

```
Name: Radio Station
Image: AI-generated
Description: AI-generated
Location reference: A pirate radio station in Le Marais — community night work, unlocked day 8 / 35 reputation
```

```
Name: Caveau des Poètes
Image: AI-generated
Description: AI-generated
Location reference: An open mic venue in Le Marais — social night stage, unlocked day 10 / 40 reputation
```

### Saint-Germain

```
Name: Agence du Quartier
Image: AI-generated
Description: AI-generated
Location reference: The landlord's office — admin errand for rent prepayment, unlocked day 6 / 25 reputation
```

```
Name: Sato's Studio
Image: AI-generated
Description: AI-generated
Location reference: Sato's breathwork studio — rival work, unlocked day 12 / 45 reputation
```

```
Name: Vermillion
Image: AI-generated
Description: AI-generated
Location reference: Alex's cocktail bar — rival night work, unlocked day 14 / 50 reputation
```

### Paris Edges

```
Name: Père Lachaise Garden
Image: AI-generated
Description: AI-generated
Location reference: A memorial garden at the cemetery outskirts — outdoor quiet and spiritual volunteering, unlocked day 15 / 55 reputation
```

```
Name: Saint-Denis Basilica Crypt
Image: AI-generated
Description: AI-generated
Location reference: Temple ruins at Saint-Denis — outdoor spiritual pilgrimage, unlocked day 18 / 60 reputation
```

```
Name: Fontainebleau Retreat
Image: AI-generated
Description: AI-generated
Location reference: A mountain retreat in Fontainebleau — spiritual pilgrimage and rest, unlocked day 21 / 70 reputation
```

```
Name: House of Middleway
Image: AI-generated
Description: AI-generated
Location reference: Brian's chapel at the outskirts — spiritual community outdoors, day-one welcome then day 6 / 15 reputation
```

---

## Portrait generation reference

The standard prompt template for regenerating portraits lives at
[docs/PORTRAIT_REFERENCE.md](docs/PORTRAIT_REFERENCE.md). Edit it freely — it is
the single source of truth for the house style.

---

## Sex/portrait mismatches

Characters whose recorded sex does not match their current portrait. These are
flagged for regeneration using the reference above.

| # | Character | Sex | Current portrait shows | Reason |
|---|-----------|-----|------------------------|--------|
| 1 | Cheezl | Male | Woman | Wrong sex |
| 2 | Lou | Female | Elderly man | Wrong sex |
| 3 | Sato | Male | Woman | Wrong sex |
| 4 | Yume | Male | Woman | Wrong sex |
| 5 | Qusтoge | Male | Woman | Wrong sex |
| 6 | Self | Male | Woman | Wrong sex |
| 7 | diamndsdancin | Male | Woman | Wrong sex |
| 8 | Kopung (고풍) | Male | Elderly woman | Wrong sex |
| 9 | Isra | Male | Woman | Wrong sex |
| 10 | Scatmandu | Male | Black woman | Wrong sex |
| 11 | Cat | Male | Actual cat (animal) | Should be human male — **RESOLVED 2026-07-29** |
| 12 | HawkinsTV | Other (robot) | Human male | Should be robot — **RESOLVED 2026-07-29** |

When a portrait is regenerated to match, move the row into the "Resolved" table
below.

### Resolved

| Character | Sex | Regenerated to | Date |
|-----------|-----|----------------|------|
| Cheezl | Male | Male at night market food stall | 2026-07-29 |
| Lou | Female | Elderly woman at Bibliothèque Forney | 2026-07-29 |
| Sato | Male | Japanese man at breathwork studio | 2026-07-29 |
| Yume | Male | Japanese man on Paris rooftop at dusk | 2026-07-29 |
| Qusтoge | Male | Man reading at Bibliothèque Forney | 2026-07-29 |
| Self | Male | Man meditating at House of Middleway | 2026-07-29 |
| diamndsdancin | Male | Man at dance studio in Saint-Germain | 2026-07-29 |
| Kopung (고풍) | Male | Elderly Korean man at pottery wheel | 2026-07-29 |
| Isra | Male | Man at pawn shop counter | 2026-07-29 |
| Scatmandu | Male | Black man at pirate radio station | 2026-07-29 |
| Cat | Male | Human male meditating at spiritual community | 2026-07-29 |
| HawkinsTV | Other (robot) | Robot with camera head at radio station | 2026-07-29 |
| Léon | Male | Middle-long brunette hair, orthodox beard | 2026-07-29 (Issue #32) |

### Still pending

None — all 12 sex/portrait mismatches have been resolved.

---

## Summary counts

| Metric | Value |
|--------|-------|
| Total characters | 78 |
| Characters with known sex | 75 / 78 (55 Male, 16 Female, 4 Other, 2 unknown, 1 flagged for removal) |
| Characters with custom image | 1 / 78 (Brian) |
| Characters with custom profile | 0 / 78 |
| Characters with custom events | 0 / 78 |
| **Portrait/sex mismatches** | **0 / 78** — all 12 resolved |
| Total locations | 23 |
| Locations with custom image | 0 / 23 |
| Locations with custom description | 0 / 23 |

### Sex breakdown

| Sex | Count | Characters |
|-----|-------|------------|
| Male | 55 | Léon, Kaden, Sato, Alex, Geo, Lakshay, Arian, Simon, Kaj, Dorian, Barret, Ethan, Matt, Artem, Brian, RicoLewis, Yume, Brock Lee, Tarrasqu, Kaschem, Mateo, Luca, Friend, Cheezl, Sir Cruds, Qusтoge, Cary, Self, Baris, Fraghis, Mrone, 𝕽𝖆𝖚𝖑, Stephen, diamndsdancin, Seth, SiekamCebulę, Kopung, Isra, Kobideh, stijn12d, Andre Watson, Air-Vaisselle, Crveni, blokely, Jits, Gordon, Jared, RicardoEA, SpeedFire, Iulian, Brendan, Scatmandu, yungnosaj, Joar, Cat |
| Female | 16 | Klaudia, Susan, Marlies, Hanans, Vanna, Daniela, Aril Stellar☯, Alvigunilla, Marlène xoxo, Kate, Ahyeon Oh, emily, Orshi, Renata 🦥, Hazel, Lou |
| Other | 4 | HawkinsTV (robot), Carl-bot (robot), DocBot (robot), groovyphoenix (firebird) |
| unknown | 2 | Yun, nestomalt |
| Flagged for removal | 1 | Oh (misparse of "Ahyeon Oh") |
