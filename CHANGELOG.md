...
the balance contract is unchanged.

### The "fragile fraud" label leaked onto day 1

Kaden's opening smear is supposed to reveal a small `fragile fraud` line under
Léon's name on day 2. It was visible from day 1. The JS wiring was already
correct (`toggleAttribute('hidden', …)` based on `kadenSmearSeen`); the bug was
purely CSS: `.fragile-fraud { display: block }` is author origin and overrides
the browser's user-agent `[hidden] { display: none }`, so the `hidden` attribute
stopped hiding anything. jsdom's `getComputedStyle` does not model that cascade,
which is why the prior regression test (a re-derived boolean, not the DOM) never
caught it. Fixed by adding `.fragile-fraud[hidden] { display: none }`
(higher specificity, so the attribute wins again) and a CSS-structure test that
asserts the rule directly. The label also now renders **lower-case**
`fragile fraud` (`text-transform: lowercase`, was `uppercase`).

### Kaden now has a face on the smear interlude

`renderStoryModal` takes an optional lead element; the Kaden smear modal looks
up Kaden and renders his avatar + role at the top, exactly like the event
popups preview their character. Both `app.js` call sites pass `gs`.

### The first piano loop is back

The very first background track the game shipped — the warm additive-synth
piano loop (`warm_piano.wav`, generated by `scripts/gen-piano.py`) — was swapped
out for a hearth pad and then a felt-piano loop that never matched it, and the
original was deleted without instruction. It is restored as the canonical track:
`scripts/gen-piano.py` writes the 44.1 kHz master and `scripts/downsample.py`
produces the ~803 KiB, 11.025 kHz deployed copy (well inside the 1 MB lazy
budget). `comfy_piano.wav` and `scripts/gen-comfy-piano.py` are removed; the
audio wiring (`preferences-service.js`) and asset pipeline
(`optimize-assets.sh`, which now regenerates both tiers) point at `warm_piano`.
`audio.test.js` asserts the two-tier master/deploy contract.

### The hub title stopped glowing on every load

`showScreen` focuses the destination heading (`tabindex="-1"`, for screen-reader
announcement and scroll restoration) on every navigation, including the very
first hub load. With no author rule to suppress it, the browser's default focus
ring boxed in "Where will you spend today?" every time the game opened. Added
`h1/h2/h3[tabindex='-1']:focus { outline: none }` — the headings are not in the
Tab order, so real keyboard focus (on actual controls, via `:focus-visible`) is
unaffected.

### Verification

- `npm run check` is green: lint, `format:check`, typecheck, **435 tests / 0
  fail**, `check-assets` (lazy music 0.78 MB).
- Coverage gate holds: **97.53% lines / 86.00% branches / 91.09% functions**
  (all above the 80% floor).
- New/rewritten tests: real fragile-fraud CSS-structure + DOM-attribute tests
  (replacing the tautological one), the focus-ring CSS rule, Kaden portrait in
  the smear modal, and the two-tier warm-piano audio contract.
