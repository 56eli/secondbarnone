...
func select_event(
	...
	# Scheduled check: only fire if we've reached the scheduled day
...
func _build_pool(journey_day: int, weekday: int, location: String) -> Array[EventDefinition]:
	...
	for e in _all_events:
		...
		if e.required_location != "" and e.required_location != location:
			continue
		# Burnout (id match, not rarity): only eligible if high consecutive bar days
...
func _build_event_pool() -> void:
	...
	# ── Spiritual Community events (required_location = "spiritual_community") ──

	# Standard (weight 10)
	_all_events.append(_make_event("inspiring_meditation", "Inspiring Meditation",
		"The meditation session was profound. Your spirit feels renewed.",
	...
	_all_events.append(_make_event("sc_deep_meditation", "Deep Meditation Breakthrough",
		"A profound breakthrough during meditation leaves your spirit soaring. You feel truly connected.",
	...
	_all_events.append(_make_event("sc_spiritual_crisis", "Spiritual Crisis",
		"A wave of existential doubt washes over you. Nothing feels meaningful anymore.",
	...
	_all_events.append(_make_event("bar_fight_night", "Bar Fight",
		"A brawl breaks out. You get caught in the chaos and lose earnings to broken glass and a bruised spirit.",
	...
	var burnout := _make_event("burnout", "Burnout",
		"The late nights have caught up with you. Exhaustion sets in.",
