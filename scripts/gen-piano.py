...
This is the original background-music generator for secondbarnone — the warm
piano loop that shipped when sound first went in. It was later swapped out for
short-lived replacements (hearth pad, then a felt-piano loop) that never
matched it, so it is the canonical track again. It writes a 44.1 kHz master;
`scripts/downsample.py` produces the small deployed copy.
"""
